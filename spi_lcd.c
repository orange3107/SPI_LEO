...
void drawImage(char path[], int x, int y, int w, int h) {
	...
	printf("OK");
	unsigned char bmpBuffer[(w * h) * pixelSize];
	...
	fseek(pFile, 54, 0);
	fread(bmpBuffer, pixelSize, w * h, pFile);
	fclose(pFile);
	printf("OK");
	...
	for (j = 0; j < h; j++)
	{
		...
		for (i = 0; i < w; i++)
		{
			...
			rgb = *(unsigned int*)(bmpBuffer + i * 3 + (h - 1 - j) * 3 * w);

			color = Color565(((rgb >> 16) & 0xFF), ((rgb >> 8) & 0xFF), (rgb & 0xFF));

			//spilcdRectangle(i, j, 1, 1, color, color);

			buffer[ptr++] = color >> 8;
			buffer[ptr++] = color & 0xFF;
	...
	//spilcdWriteDataBlock(buffer, h*w*3);
	//buffer  = NULL;
